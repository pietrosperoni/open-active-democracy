...
class VideoWorker
    ...
    if process_discussion
      process_discussion.process_speech_videos.get_all_for_modified_duration.each do |video|
        ...
        if video==process_discussion.process_speech_videos.get_all_for_modified_duration.last and video.modified_duration_s == nil
          info "adding to 2 seconds to last video id #{video.id} in discussion id #{process_discussion.id}"
  ...
  def process_discussion
    ...
    if process_discussion
      ...
      process_discussion.save
      info "Processing Process Discussion Id: #{process_discussion.id}"
      html_doc = Nokogiri::HTML(open(process_discussion.listen_url))
      @parent = nil
      last_current = nil
      last_link = nil
      last_indent = 0
      ...
      info "No more process discussions to process"
      return false
    end
  end
  ...
  def print_timecode
    ProcessSpeechVideo.find(:all, :conditions=>"published = 1", :order=>"start_offset").each do |video|
      if video.modified_duration_s
        info "#{video.process_discussion.id} #{video.inpoint_s}-#{video.modified_outpoint_s} (#{video.modified_duration_s}s) - #{video.title}"
      else
        info "#{video.process_discussion.id} #{video.inpoint_s}-#{video.outpoint_s} (#{video.duration_s}s) - #{video.title}"
      end
    end
  end
  ...
  def convert_all_speeches_to_flash(master_video)
    ...
    master_video.process_speech_videos.each do |video|
      ...
      info "PROCESS: #{video.process_discussion.meeting_url}"
      speech_video_path = "#{RAILS_ROOT}/public/"+ENV['RAILS_ENV']+"/process_speech_videos/#{video.id}/"
      speech_video_first_tmp_filename = speech_video_path+"speech.tmp_1.flv"
