...
#    action_links action_link("Svara","{*comment_url*}")      
  ...
  def comment(facebook_session, comment, activity)
    ...
    send_as :publish_stream
    message = "#{facebook_session.user.name} skrifaði athugasemd við #{object_name} á Skuggaþingi: #{truncate(comment.content, :length => 400)}"
    from facebook_session.user
    message ''
    attachment :name => object_name, :href => object_url, :description => message
...
#    action_links action_link("Skoða betur","{*point_url*}")      
  ...
  def point(facebook_session, point, priority)
    send_as :publish_stream
    message = "#{facebook_session.user.name} bætti við rökum við #{priority.name} á Skuggaþingi: #{point.name_with_type} - #{point.content}"
    from facebook_session.user
    message ''
    attachment :name => point.name_with_type, :href => point.show_url, :description => message
    action_links [ :text => 'Rökræða mál', :href => "#{priority.show_url}/top_points"]
...
#    action_links action_link("Skoða betur","{*document_url*}")  
  ...
  def document(facebook_session, document, priority)
    send_as :publish_stream
    message = "#{facebook_session.user.name} bætti við skjali við #{priority.name} á Skuggaþingi: #{document.name_with_type} - #{truncate(document.content, :length => 400)}"
    from facebook_session.user
    message ''
    attachment :name => document.name_with_type, :href => document.show_url, :description => message
    action_links [ :text => 'Rökræða mál', :href => "#{priority.show_url}/top_points"]
