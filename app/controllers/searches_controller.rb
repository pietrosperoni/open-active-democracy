  ...
  def index
    ...
    if params[:q]
      ...
      if query.blank?
        ...
        @priority_results = Priority.find_by_solr "(" + params[:q] + "~0.8) AND is_published:true", :offset => ((params[:page]||1).to_i-1)*25, :limit => 25
        @priorities = @priority_results.docs
        
        @document_results = Document.find_by_solr "(" + params[:q] + "~0.8) AND is_published:true", :offset => ((params[:page]||1).to_i-1)*25, :limit => 1
        @documents = @document_results.docs
        
        @point_results = Point.find_by_solr "(" + params[:q] + "~0.8) AND is_published:true", :offset => ((params[:page]||1).to_i-1)*25, :limit => 1
  ...
  def points
    ...
    if params[:q]
      ...
      if query.blank?
        ...
        @priority_results = Priority.find_by_solr "(" + params[:q] + "~0.8) AND is_published:true", :offset => ((params[:page]||1).to_i-1)*25, :limit => 1
        @priorities = @priority_results.docs
        
        @document_results = Document.find_by_solr "(" + params[:q] + "~0.8) AND is_published:true", :offset => ((params[:page]||1).to_i-1)*25, :limit => 1
        @documents = @document_results.docs
        
        @point_results = Point.find_by_solr "(" + params[:q] + "~0.8) AND is_published:true", :offset => ((params[:page]||1).to_i-1)*25, :limit => 15
        @points = @point_results.docs
        @qualities = nil
  ...
  def documents
    ...
    if params[:q]
      ...
      if query.blank?
        ...
        @document_results = Document.find_by_solr "(" + params[:q] + "~0.8) AND is_published:true", :offset => ((params[:page]||1).to_i-1)*25, :limit => 15
        @documents = @document_results.docs
