namespace :trans do  
   ...
   task :download do
     ...
     config.keys.each do |f|
       ...
       tr.keys.each do |t|
         ...
         download_file('99translations.com', "/download/#{api_key}/#{f}/#{t}", File.join(Rails.root, tr[t]))
       end 
     end
   end
   ...
   task :update_all do
      plugins = File.join(Rails.root, 'vendor', 'plugins')
   ...
   def merge_translations(locale, hash)
     file = File.join(Rails.root, 'config', 'locales')
   ...
   def config_file
      File.join(Rails.root, 'config', 'trans.yml')
   end
