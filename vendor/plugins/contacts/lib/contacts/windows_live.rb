...
module Contacts
  ...
  class WindowsLive
    ...
    def initialize(config_file=CONFIG_FILE)
      if Government.last and Government.last.attribute_present?("windows_appid")
        @wll = WindowsLiveLogin.new(Government.last.windows_appid, Government.last.windows_secret_key, 'wsignin1.0',nil, 'http://' + Government.last.base_url + '/about/privacy', 'http://' + Government.last.base_url + '/import/windows')        
      else
        confs = YAML.load_file(config_file)['windows_live']
        @wll = WindowsLiveLogin.new(confs['appid'], confs['secret'], confs['security_algorithm'],nil, 'http://' + Government.last.base_url + '/about/privacy', 'http://' + Government.last.base_url + '/import/windows')
      end      
    end
